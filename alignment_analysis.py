...
class AlignmentAnalyzer:
    ...
    def add_noise_at_timestep(
        self, clean_images: torch.Tensor, timestep: float
    ...
    def compute_single_timestep_gradients(
        self, images: torch.Tensor, timestep: float, text_prompt: list[str] | None = None
        ...
        for name, param in self.model.named_parameters():
            if param.grad is not None:
                gradients[name] = param.grad.flatten().cpu()
        self.model.zero_grad()
        return gradients

    def compute_pairwise_score(
        self, images: torch.Tensor, timestep_1: float, timestep_2: float, text_prompt: list[str] | None = None
    ) -> float:
        """
        Compute alignment score between two timesteps using sum of parameter-wise cosine similarities.

        For computational efficiency and better semantic meaning, this computes cosine similarity
        between corresponding parameters separately and sums them, rather than concatenating
        all gradients into single vectors. This gives equal weight to each parameter group
        regardless of size.

        Args:
            images: Batch of clean images [B, 3, H, W] in [-1, 1] range
            timestep_1: First timestep value in [0, 1] range
            timestep_2: Second timestep value in [0, 1] range
            text_prompt: Optional text prompts (uses dummy if None)

        Returns:
            Sum of cosine similarity scores between corresponding parameter gradients
        """
        # Compute gradients for both timesteps
        gradients_1 = self.compute_single_timestep_gradients(images, timestep_1, text_prompt)
        gradients_2 = self.compute_single_timestep_gradients(images, timestep_2, text_prompt)

        # Compute cosine similarity for each corresponding parameter and sum them
        total_similarity = 0.0
        for param_name in gradients_1.keys():
            if param_name in gradients_2:
                param_sim = F.cosine_similarity(
                    gradients_1[param_name].unsqueeze(0), gradients_2[param_name].unsqueeze(0), dim=1
                )
...
def run_single_denoising_step(
    model, ae, t5, clip, images: torch.Tensor, device: str, timestep: float | None = None
) -> Dict[str, Any]:
...
def main(config: Config):
    ...
    dataloader = load_imagenet_dataset(
        config.imagenet_path, batch_size=config.batch_size, num_workers=config.num_workers
    )
    print()
